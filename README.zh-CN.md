...
一坐下就忘了时间？说好再忙五分钟，回过神来屁股已经和椅子达成长期合作。

**起来歇会儿**会到点提醒你站起来、伸伸懒腰，让眼睛离开屏幕。平时安静待在托盘里，休息时照顾到每一块屏幕——别想换个屏幕继续卷。

默认每 60 分钟提醒休息 10 秒，忙多久、歇多久都由你设置。下载后运行 EXE 即可，无需安装；支持 Windows 10/11 x64。

## 功能

- 平时仅显示托盘图标，不占任务栏。
- 打开设置即可查看距离下次休息的倒计时，以及暂停或等待解锁等运行状态。
...
设置窗口的倒计时显示当前已生效的提醒计划，每秒刷新；尚未保存的修改不会影响它。设置打开期间不会弹出休息提醒，到点会跳过本次休息并重新计时。查看完请关闭设置，让它继续在托盘里陪你工作。
...
采用 **C++17 + 原生 Win32 API**，无需 Qt、Electron 或 .NET。
